...
window.GameConstants = {
  Fireball: {
    size: window.setup.fireballSize || 24,
    speed: window.setup.getFireballSpeed || function (movingLeft) {
      return movingLeft ? 2 : 5;
    }
  },
  Wizard: {
    speed: window.setup.wizardSpeed || 2,
    width: window.setup.wizardWidth || 61,
    getHeight: window.setup.getWizardHeight || function (width) {
      return 1.377 * width;
    },
    getX: window.setup.getWizardX || function (width) {
      return width / 3;
    },
    getY: window.setup.getWizardY || function (height) {
      return height - 100;
    }
  }
};
